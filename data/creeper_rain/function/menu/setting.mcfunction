...
tellraw @s {"text":" [修改Creeper落地后的引爆时间]","color":"aqua","hoverEvent": {"action": "show_text","value":"\u00A7d请在\"value\":后的双引号内键入数值\u00A7b \n(示例:30tick(1.5s)为默认引爆速度)"},"clickEvent": {"action": "suggest_command","value": "/function creeper_rain:setting/set_time {\"time\":\"\"}"}}
